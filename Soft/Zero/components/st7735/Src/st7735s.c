...
void lcdInit(ST7735_t * dev, int width, int height)
{
	...
	spi_master_write_data_byte(dev, 0x68);	//BGR color filter panel
...
void lcdDrawPixel(ST7735_t * dev, uint16_t x, uint16_t y, uint16_t color){
	...
	spi_master_write_addr(dev, _y+25, _y+25);
...
void lcdDrawFillRect(ST7735_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) {
	...
	spi_master_write_command(dev, 0x2A);	// set column(x) address
	spi_master_write_addr(dev, _x1+25, _x2+25);
	spi_master_write_command(dev, 0x2B);	// set Page(y) address
...
// Sleep in
void lcdSleepIn(ST7735_t * dev) {
	spi_master_write_command(dev, 0x10);	//Display off
}

// Sleep out
void lcdSleepOut(ST7735_t * dev) {
	spi_master_write_command(dev, 0x11);	//Display off
}
...
void st7735s_send_color(ST7735_t * dev,void * data, uint16_t length)
{
	spi_master_write_byte( dev->_SPIHandle, data, length);
}

// void lcdSetPixel(ST7735_t * dev, uint16_t x, uint16_t y)
// {
// 	if (x >= dev->_width) return;
// 	if (y >= dev->_height) return;
...
// 	uint16_t _y = y + ROWSTART;
// 	spi_master_write_command(dev, 0x2A);	// set column(x) address
// 	spi_master_write_addr(dev, _x, _x+160);
// 	spi_master_write_command(dev, 0x2B);	// set Page(y) address
// 	spi_master_write_addr(dev, _y+25, _y+25);
// 	spi_master_write_command(dev, 0x2C);	//  Memory Write
// }

void lcdColor_Fill(ST7735_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t* color) 
{
	uint16_t i,j;
	uint8_t Byte[2];
	if (x1 >= dev->_width) return;
	...
	spi_master_write_command(dev, 0x2B);	// set Page(y) address
	spi_master_write_addr(dev, _y1+25, _y2+25);
	spi_master_write_command(dev, 0x2C);	//  Memory Write
	...
	for(i=x1;i<=x2;i++)
	{
		for(j=y1;j<=y2;j++)
		{
			Byte[0] = (*color >> 8) & 0xFF;
	        Byte[1] = *color & 0xFF;
			spi_master_write_byte( dev->_SPIHandle, Byte, 2);
			color++;
		}
	}
...
void lcdColor_Fill1(ST7735_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t* color) 
{
	uint16_t i,j = 0;
	uint8_t Byte[(x2-x1+1)*(y2-y1+1)*2];
	...
	spi_transaction_t SPITransaction;
	memset( &SPITransaction, 0, sizeof( spi_transaction_t ) );
	SPITransaction.length = (j-1) * 8;
	SPITransaction.tx_buffer = Byte;
	spi_device_polling_transmit( dev->_SPIHandle, &SPITransaction );
}
