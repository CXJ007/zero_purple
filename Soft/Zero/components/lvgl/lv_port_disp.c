...
void lv_port_disp_init(void)
{
    ...
    static lv_color_t draw_buf_1[LV_HOR_RES_MAX* 5];                          /*A buffer for 10 rows*/
    lv_disp_buf_init(&draw_buf_dsc_1, draw_buf_1, NULL, LV_HOR_RES_MAX*5);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    //         lcdDrawPixel(&dev, (uint16_t) x, (uint16_t) y, color_p->full);
    //         color_p++;
    //     }
    // }

    lcdColor_Fill1(&dev, area->x1, area->y1, area->x2, area->y2, (uint16_t*)color_p);
     /* IMPORTANT!!!
